...
- Retry recovery must resolve the configured tracker project first and then treat retained issues as belonging to that project only when their tracker payload reports the same canonical Linear `slugId`.
...
- If the leased issue becomes terminal during a daemon tick, `maestro` must stop the active run, mark the attempt `terminated`, clear the lease, and then retain or clean the workspace according to the retention rules above.
...
- The operator snapshot transport must remain read-only. When `[operator_http]` is configured, daemon mode may additionally expose that same snapshot model from an HTTP `GET /state` endpoint, but the listener must stay disabled by default when no operator HTTP config is present.
