	...
	command
		...
		.args(
			request
				...
				.map(|state| vec!["--initial-issue-state", state])
				.unwrap_or_default(),
		)
		.args([
			"--dispatch-mode",
	...
	I: PullRequestReviewStateInspector,
{
	...
	if mergeability_requires_review_repair(&review_state.mergeable) {
		classification.decision = PostReviewLaneDecision::NeedsReviewRepair;
		classification.reason = String::from("pull_request_merge_conflict");
...
fn mergeability_requires_review_repair(mergeable: &str) -> bool {
	matches!(mergeable, "CONFLICTING")
}
...
{workflow_body}"#
	...
	fn schedule_retry_after_child_exit_records_continuation_retry_for_clean_exit() {
		...
		state_store
			.record_run_attempt(run_id, &issue.id, 1, orchestrator::CONTINUATION_PENDING_RUN_STATUS)
			.expect("run attempt should record");
